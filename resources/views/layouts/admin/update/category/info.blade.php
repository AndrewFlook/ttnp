<h5 class="card-header">
	...
	<form class="float-right" method="POST" action="{{ route('admin.categories.destroy', $category->id) }}">
		@method('DELETE')
		@csrf
...
<div class="card-body">
	<form id="categoryForm" role="form" method="POST" action="{{ route('admin.categories.update', $category->id) }}">
		@method('PATCH')
		@csrf
		...
		<div class="form-group row">
			...
			<div class="col-sm-10">
				<input type="text" class="form-control" id="desc" name="desc" placeholder="Category Description" value="{{ $category->desc }}">
			</div>
		</div>
		<div class="form-group row">
			<label for="desc_options" class="col-sm-2 col-form-label">Description of available options</label>
			<div class="col-sm-10">
				<input type="text" class="form-control" id="desc_options" name="desc_options" placeholder="Description of available options" value="{{ $category->desc_options }}">
			</div>
		</div>

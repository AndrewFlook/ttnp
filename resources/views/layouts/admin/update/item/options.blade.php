<h5 class="card-header" style="border-top:1px solid rgba(0,0,0,.125)">
	...
	<small class="text-muted ml-2">
		@if(count($item->options) == 0)
			There are currently <strong>NO options</strong> available for this item.
		@elseif(count($item->options) == 1)
			<strong>1 option</strong> available for this item:
		@elseif(count($item->options) >= 2)
			<strong>{{ count($item->options) }} options</strong> available for this item:
		@endif
	</small>

<h5 class="card-header" style="border-top:1px solid rgba(0,0,0,.125)">
	...
	<small class="text-muted ml-2">
		@if(count($item->prices) == 0)
			There are currently <strong>NO prices</strong> assigned to this item.
		@elseif(count($item->prices) == 1)
			<strong>1 price</strong> assigned to this item:
		@elseif(count($item->prices) >= 2)
			<strong>{{ count($item->prices) }} prices</strong> assigned to this item:
		@endif
	</small>
